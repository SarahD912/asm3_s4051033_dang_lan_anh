...
let threadImage;
// New variable to hold the button image
let menuButtonImage;
...
// NEW: Variables for the menu button
let menuIsVisible = false;
let menuButtonSize = 50;
let menuButtonPadding = 20;
...
function draw() {
  ...
  // --- 2. Draw interactive circles and red wires ONLY if NOT in the final state and if the menu is not visible ---
  if (!finalState && !menuIsVisible) {
  ...
  if (rectangleIsVisible) {
    ...
    textSize(20);
    text(
      "You have made your choice. I know it's hard but you can't stop here yet. ",
      width / 2,
      height / 2 + 20
    );
  }
  ...
  if (finalState) {
    ...
    line(
      0,
      height,
      glowingRectX - glowingRectWidth / 2,
      glowingRectY + glowingRectHeight / 2
    );
    ...
    if (
      glowingRectWidth >= width &&
      glowingRectHeight >= height &&
      !textTriggered
    ) {
      textTriggered = true;
      startTyping();
  ...
  // NEW: Draw the hands and central image ONLY if not in the final text state
  if (!textTriggered) {
    ...
    // Both hands and the central image are drawn at the same position
  ...
  // NEW: Draw the menu content if the menu is visible, overriding everything else
  if (menuIsVisible) {
    drawMenuContent();
  }

  // NEW: Draw the menu button at the top-left corner on top of everything
  // This ensures it's always visible and clickable
  drawMenuButton();
}
...
function drawMenuButton() {
  image(
    menuButtonImage,
    menuButtonPadding + menuButtonSize / 2,
    menuButtonPadding + menuButtonSize / 2,
    menuButtonSize,
    menuButtonSize
  );
...
function drawMenuContent() {
  ...
  // 1. Menu heading
  textSize(32);
  text("MENU", width / 2, currentY);
  currentY += 40;

  // 2. Project introduction
  textSize(18);
  text("Welcome to my project:", width / 2, currentY);
  currentY += 50; // Increased spacing

  // NEW: Project title is now bold and larger
  textSize(24); // Make the text bigger
  textStyle(BOLD); // Make the text bold
  text(
    "“How my hands' potential hold my future carrier?”",
    width / 2,
    currentY
  );
  textStyle(NORMAL); // Reset text style to normal for other text
  currentY += 50; // Add extra space for a paragraph break

  // 3. Phase 1 Instructions
  textSize(20);
  text("Phase 1: Dragging and Clicking", width / 2, currentY);
  currentY += 45; // Increased spacing
  textSize(16);
  text(
    "Use MOUSE to Click on any of the 7 potential carriers",
    width / 2,
    currentY
  );
  currentY += 30; // Increased spacing
  text("or hold the hands/thread ball and drag around.", width / 2, currentY);
  currentY += 30; // Increased spacing
  text(
    "After the boxchat shows up, CLICK on the boxchat.",
    width / 2,
    currentY
  );
  currentY += 50; // Increased spacing

  // 4. Phase 2 Instructions
  textSize(20);
  text("Phase 2: Moving Forward", width / 2, currentY);
  currentY += 45; // Increased spacing
  textSize(16);
  text("Press the 'W' key to move forward.", width / 2, currentY);
  currentY += 50; // Increased spacing

  // 5. Phase 3 Instructions
  textSize(20);
  text("Phase 3: Final Experience", width / 2, currentY);
  currentY += 45; // Increased spacing
  textSize(16);
  text("Use the MOUSE to Scroll up to experience.", width / 2, currentY);
  currentY += 50; // Increased spacing

  // 6. User Options
  textSize(20);
  text("Press 'R' to Restart", width / 2, currentY);
  currentY += 35; // Increased spacing
  text("Press 'M' to Close Menu", width / 2, currentY);
  currentY += 50; // Increased spacing

  // 7. Final Message
  textSize(18);
  text("Have a great experience, my dear viewers.", width / 2, currentY);
}
...
function drawTypingText() {
  // This will cover the glowing rectangle with a solid black fill
  fill(255);
  rect(width / 2, height / 2, width, height);
  ...
  fill(0);
  textAlign(CENTER, CENTER); // Align the text to the top-left of the new box
  textSize(24);
  text(displayText, width / 2, height / 2, textBoxWidth, textBoxHeight);
}
...
function mousePressed() {
  // Check if the menu button image is clicked first
  let menuButtonX = menuButtonPadding;
  let menuButtonY = menuButtonPadding;
  if (
    mouseX > menuButtonX &&
    mouseX < menuButtonX + menuButtonSize &&
    mouseY > menuButtonY &&
    mouseY < menuButtonY + menuButtonSize &&
    !finalState
  ) {
  ...
  // If the menu is open, don't allow other interactions
  if (menuIsVisible) {
    return;
  }

  // Original interaction logic below
  if (
...
function keyPressed() {
  ...
  if (
    (isNoiseFadingIn || noiseIsPermanent) &&
    (key === "w" || key === "W") &&
    !menuIsVisible
  ) {
